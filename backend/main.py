...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        from data_layer.core.cache import init_redis as init_data_layer_redis
        await init_data_layer_redis()
        await data_layer_simulator.start()
        logger.info("Local the internal simulation engine data layer simulation manager started successfully")
    except Exception as e:
        logger.error(f"Failed to start local data layer simulation manager: {e}")
    ...
    # ── Load NSE/BSE equity symbols from the internal simulation engine data feed ────────
    try:
        from services.contract_loader import get_nse_contracts_cached, fetch_exchange_symbols
        from providers.contract_symbol_map import register_exchange_symbols

        # Fetch NSE symbols (cached for on-demand symbol registration)
        nse_contracts = await get_nse_contracts_cached()
        if nse_contracts:
            nse_count = register_exchange_symbols(nse_contracts)
            logger.info(f"the internal simulation engine NSE symbols loaded: {nse_count} equity symbols")
        else:
            logger.warning("the internal simulation engine NSE symbols unavailable (data feed not configured/enabled?)")

        # Fetch BSE symbols for SENSEX constituents and other BSE stocks
        bse_contracts = await fetch_exchange_symbols("BSE", segment="EQ")
        if bse_contracts:
            bse_count = register_exchange_symbols(bse_contracts)
            logger.info(f"the internal simulation engine BSE symbols loaded: {bse_count} equity symbols")
        else:
            logger.warning("the internal simulation engine BSE symbols unavailable")

        if not (nse_contracts or bse_contracts):
            logger.warning(
                "the internal simulation engine symbol lists unavailable — "
                "symbols will be resolved on-demand or fall back to the local simulator"
            )
    except Exception as e:
        logger.warning(
            f"the internal simulation engine symbol load failed — on-demand resolution will handle it: {e}"
        )

    # ── Load MCX/NCDEX commodity symbols (near-expiry FUT contracts) ────────
    try:
        from services.contract_loader import fetch_commodity_contracts
        from providers.contract_symbol_map import register_exchange_symbols as _register_commodity_symbols

        commodity_contracts = await fetch_commodity_contracts()
        if commodity_contracts:
            count = _register_commodity_symbols(commodity_contracts)
            logger.info(
                f"MCX/NCDEX commodity symbols loaded: {count} symbols pre-registered"
            )

            # Dump full commodity symbol map for live debugging
            try:
                from providers.contract_symbol_map import dump_commodity_symbol_map
                dump_commodity_symbol_map()
            except Exception:
                pass
        else:
            logger.warning(
                "MCX/NCDEX commodity symbols unavailable from the internal simulation engine — "
                "commodity quotes will fall back to the local simulator"
            )
    except Exception as e:
        logger.warning(f"MCX/NCDEX commodity symbol load failed: {e}")

    # ── Load NSE futures contracts (real expiries/OI from the internal simulation engine) ────────
    try:
        from services.futures_service import initialize_futures

        await initialize_futures()
        logger.info("NSE futures contracts loaded from the internal simulation engine")
    ...
    # ── Start the master data feed session ──────────────────────────
    # Single shared provider for all users (the internal simulation engine if admin-configured
    # and enabled, else the local simulator). No per-user broker sessions.
    try:
        restored = await data_feed_session_manager.restore_sessions()
        logger.info(
            f"Data feed session initialized | "
            f"Single shared provider — no per-user broker auth required"
        )
    except Exception as e:
        logger.error(f"Data feed session restore failed: {e!r}", exc_info=True)

    # ── Startup diagnostics ────────────────────────────────────────────
    logger.info("=" * 70)
    logger.info("ALPHASYNC STARTUP DIAGNOSTICS")
    logger.info("=" * 70)
    logger.info("Data feed model: single shared provider (the internal simulation engine or local simulator)")

    # Check data feed session
    active_session_count = data_feed_session_manager.session_count()
    logger.info(f"Active Data Feed Sessions: {active_session_count}")
    ...
    logger.info("- Options data: ENABLED (internal simulation engine, simulated fallback)")
    logger.info("- Commodities: ENABLED (internal simulation engine with local simulator fallback)")
    logger.info("=" * 70)
    ...
    await data_feed_session_manager.stop()
    try:
        from data_layer.simulator.simulator_manager import simulator_manager as data_layer_simulator
        from data_layer.core.cache import close_redis as close_data_layer_redis
        await data_layer_simulator.stop()
        await close_data_layer_redis()
...
from data_layer.api.routes_sessions import router as data_layer_sessions_router
from data_layer.api.routes_feed import router as data_layer_feed_router
from data_layer.api.routes_price import router as data_layer_price_router
from data_layer.api.routes_symbols import router as data_layer_symbols_router
from data_layer.api.routes_admin import router as data_layer_admin_router
from data_layer.api.routes_admin import admin_keys_router as data_layer_admin_keys_router
...
app.include_router(data_layer_sessions_router)
app.include_router(data_layer_feed_router)
app.include_router(data_layer_price_router)
app.include_router(data_layer_symbols_router)
app.include_router(data_layer_admin_router)
app.include_router(data_layer_admin_keys_router)
