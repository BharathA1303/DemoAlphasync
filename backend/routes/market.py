...
def _build_market_unavailable_detail() -> dict:
    ...
    active_sessions = data_feed_session_manager.session_count()

    if active_sessions == 0:
        reason = "No active data feed session"
    else:
        reason = "Data feed provider temporarily unavailable"
...
async def _try_recover_any_session() -> bool:
    """Try to recover a data feed session with cooldown to avoid request storms."""
    global _last_session_recover_attempt

    from services.data_feed_session import data_feed_session_manager

    if data_feed_session_manager.get_any_session() is not None:
        return True
    ...
    return data_feed_session_manager.get_any_session() is not None
...
async def get_quote(
    ...
    SmartCache (in-memory) → Redis → data feed provider.
    """
    ...
    if data_feed_session_manager.get_any_session() is None:
...
async def get_history(
    ...
    """Historical OHLCV — SmartCache → Redis → data feed provider only."""
    fmt_symbol = market_data._format_symbol(symbol)
...
async def get_ticker(user: Optional[User] = Depends(get_current_user_optional)):
    """
    Ticker bar — SmartCache → Redis → data feed provider.
    """
    try:
...
async def get_public_ticker():
    """
    Public ticker — NO auth required. SmartCache → Redis → data feed provider.
    """
...
async def batch_quotes(
    ...
    Parallel market_data lookup: data feed provider batch only
    """
...
async def provider_health(user: User = Depends(get_current_user)):
    ...
    provider = data_feed_session_manager.get_session(user.id)
    if not provider:
        return {
            "status": "not_connected",
            "message": "No personal data feed session connected — using master session",
        }
    try:
...
async def system_diagnostics(user: Optional[User] = Depends(get_current_user_optional)):
    ...
    if user:
        try:
            user_session = data_feed_session_manager.get_session(user.id)
        except Exception:
            pass

    # Check general session availability
    any_session = data_feed_session_manager.get_any_session() is not None
    ...
    options_live_available = any_session
    ...
    return {
        ...
        "availability": {
            ...
            "options": {
                "source": "tickalpha",
                "data_feed_available": options_live_available,
                "status": "enabled" if options_live_available else "unavailable",
            },
            "commodities": {
                "data_feed_required": False,
                "data_feed_available": any_session,
                "fallback": "cache_only",
                "status": "enabled",
            },
        },
        "recommendations": _generate_recommendations(
            user_session, any_session, total_futures
        ),
    }
...
def _generate_recommendations(
    ...
    if not any_session:
        recs.append(
            {
                "severity": "warning",
                "issue": "No active data feed session for live market data",
                "recommendation": "Configure the internal simulation engine data feed credentials in .env",
            }
        )
        ...
        recs.append(
            {
                ...
                "recommendation": "Check the internal simulation engine CDN access or verify API credentials",
            }
        )
    ...
    if not any_session:
        recs.append(
            {
                "severity": "warning",
                "issue": "No active the internal simulation engine session for options",
                "recommendation": "Restore the internal simulation engine data feed session to receive live options data",
            }
        )
