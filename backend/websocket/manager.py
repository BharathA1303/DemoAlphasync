...
class ConnectionManager:
    ...
    def subscribe(self, connection_id: str, symbols: list[str]):
        ...
        for symbol in symbols:
            ...
            # live data feed symbol also find subscribers on the legacy name.
            try:
        ...
        # Trigger data feed WS subscription immediately for newly tracked symbols
    ...
    async def _provider_subscribe(self, symbols: list[str]) -> None:
        """Forward subscribe to data feed provider for immediate WS tick delivery."""
        try:
    ...
    async def subscribe_futures(self, connection_id: str, contract_symbol: str):
        """Subscribe to a futures contract and trigger data feed WS subscription."""
    ...
    async def _provider_subscribe_futures(self, contract_symbols: list[str]) -> None:
        """Forward futures contract subscribe to data feed provider for live ticks."""
        try:
            from services.data_feed_session import data_feed_session_manager
