...
Reads prices from any available data feed provider session and emits
...
Per-user architecture:
    - No global provider. Worker uses data_feed_session_manager.get_any_session().
    - If no sessions exist, the worker idles (no data to stream).
    - When a user connects their data feed session, a session appears and the
...
from providers.contract_symbol_map import is_commodity_symbol
...
class MarketDataWorker:
    """
    Fetches live prices from any available data feed session and emits events.
    ...
    NO_SESSION_INTERVAL = 10  # seconds when no data feed sessions
    ...
    async def run(self) -> None:
        ...
        while self._running:
            try:
                ...
                if provider is None:
                    # No data feed sessions — keep existing cache until expiry.
                    self._stats["no_session_waits"] += 1
                    if self._stats["no_session_waits"] % 30 == 1:
                        logger.debug(
                            "MarketDataWorker: No data feed sessions, "
