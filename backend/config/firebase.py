...
def init_firebase() -> None:
    ...
    try:
        # Determine path to firebase-credentials.json automatically if not specified
        credentials_path = settings.FIREBASE_CREDENTIALS_PATH
        if not credentials_path and not settings.FIREBASE_CREDENTIALS_JSON:
            backend_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            p1 = os.path.join(backend_dir, "firebase-credentials.json")
            p2 = os.path.join(os.path.dirname(backend_dir), "firebase-credentials.json")
            if os.path.isfile(p1):
                credentials_path = p1
                logger.info(f"Auto-detected Firebase credentials at: {p1}")
            elif os.path.isfile(p2):
                credentials_path = p2
                logger.info(f"Auto-detected Firebase credentials at: {p2}")
        ...
        if settings.FIREBASE_CREDENTIALS_JSON:
            ...
            if google_app_creds and os.path.isfile(google_app_creds):
                try:
                    firebase_admin.initialize_app()
                    _credentials_available = True
                    logger.info(f"✓ Firebase Admin SDK initialized from GOOGLE_APPLICATION_CREDENTIALS: {google_app_creds}")
                except Exception as e:
                    logger.error(f"✗ Firebase initialization failed using GOOGLE_APPLICATION_CREDENTIALS: {e}")
                    raise
            else:
                logger.warning(
                    "⚠ No Firebase credentials found in FIREBASE_CREDENTIALS_JSON, FIREBASE_CREDENTIALS_PATH, or GOOGLE_APPLICATION_CREDENTIALS.\n"
                    "   Token verification using Firebase Admin SDK will NOT work.\n"
                    "   For localhost: Set FIREBASE_CREDENTIALS_PATH in .env or save firebase-credentials.json in root."
                )
...
def verify_firebase_token(id_token: str) -> Optional[dict]:
    ...
    try:
        ...
        # Disable revocation checks by default to remove external network dependencies.
        try:
