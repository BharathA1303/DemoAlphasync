...
class MarketSessionEngine:
    ...
    def get_session_info(self) -> dict:
        ...
        return {
            ...
            "live_feed_active": self._is_live_feed_active(),
    ...
    def _is_live_feed_active(self) -> bool:
        """True when the master market provider is the internal simulation
        engine (vs the local ReplayProvider). Drives the frontend 'Live Feed'
        vs 'Simulated Feed' badge without depending on simulation_mode (which
        stays on for both, since the internal engine is itself a replay
        stream)."""
        try:
            from services.data_feed_session import data_feed_session_manager
            provider = data_feed_session_manager.get_any_session()
            return type(provider).__name__ == "InternalSimProvider"
        except Exception:
            return False
