...
At OPEN → CLOSED transition, fetches the internal simulation engine EOD/historical closes and writes authoritative
...
async def _fetch_official_eod_from_provider(symbol: str, provider) -> Optional[dict]:
    """Fetch latest daily EOD candle from the internal simulation engine historical API."""
    try:
...
async def fetch_official_eod_close(symbol: str, provider=None) -> Optional[dict]:
    """Resolve official close: live the internal simulation engine EOD API → validated Redis daily → None."""
    ...
    *,
    ...
    async with _RECONCILE_LOCK:
        ...
        try:
            ...
            logger.warning(
                "[EOD RECONCILIATION] no data feed session — using Redis history fallback only"
            )
