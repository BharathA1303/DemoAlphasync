"""
Market Data Service — live-data-first market data.

Primary market data sources:
    1. User's personal data feed session (if connected)
    2. Master data feed account (shared live feed if configured in .env)
    3. Redis snapshots populated from prior live data feed data
...
Responsibilities:
    ...
    * Stock search (local NSE list + data feed SearchScrip API)
...
def _build_commodity_universe() -> list[dict]:
    ...
    for item in POPULAR_COMMODITIES:
        ...
        try:
            mapping = canonical_to_exchange_symbol(symbol)
        except Exception:
            mapping = None

        if mapping:
            entry["contract_symbol"] = (
                str(mapping.get("symbol") or symbol).upper().strip() or symbol
            )
            entry["exchange"] = (
...
def _convert_monotonic_volumes_to_deltas(candles: list) -> list:
    """Convert a monotonically-increasing running-total volume series
    (common for index intraday feeds) into per-bar volume deltas.

    If the series isn't actually monotonic (i.e. it's already per-bar
    volume), returns the candles unchanged.
    """
    if not candles:
        return candles

    volumes = [int(c.get("volume") or 0) for c in candles]
    is_monotonic = all(b >= a for a, b in zip(volumes, volumes[1:]))
    if not is_monotonic or volumes == [0] * len(volumes):
        return candles

    converted = []
    prev_cum = 0
    for candle, cum in zip(candles, volumes):
        delta = max(0, cum - prev_cum)
        converted.append({**candle, "volume": delta})
        prev_cum = cum
    return converted
...
def _normalize_quote(quote: Optional[dict]) -> Optional[dict]:
    ...
    # These are provided by the live data feed's touchline ticks / quote REST.
...
def _get_any_provider():
    ...
    provider = data_feed_session_manager.get_any_session()
    if provider is None:
        raise RuntimeError("No active data feed sessions — market data unavailable")
    return provider
...
async def _get_any_provider_live(allow_recover: bool = True):
    ...
    if (
        ...
        return data_feed_session_manager.get_any_session()
...
async def _enrich_frozen_quote_day_change(
    ...
    # During market hours, last_price always carries prev_close (from the feed's "c" field),
...
async def _get_frozen_quote_snapshot(symbol: str) -> Optional[dict]:
    ...
    Priority (same source family as charts — the internal simulation engine daily EOD, not stale intraday Redis):
      1. official_eod_close already in Redis last_price
      2. Live the internal simulation engine EOD / get_historical_data (writes official close to Redis)
    ...
    try:
        ...
        # Fetch fresh EOD from the internal simulation engine (charts use this path — ticker/watchlist must match).
...
async def _get_closed_session_quote(symbol: str) -> Optional[dict]:
    ...
    if history_quote:
        ...
        # pull prev_close from the official EOD snapshot — which fetches from the internal simulation engine when
...
async def get_quote(symbol: str, user_id: str) -> dict:
    """
    Get real-time quote for a symbol via the user's data feed session.
...
async def get_system_quote_safe(symbol: str) -> Optional[dict]:
    """System-level quote using the master data feed or any active provider session."""
    fmt = _format_symbol(symbol)
    market_frozen = _is_market_frozen()
...
async def get_system_quote_live_only(
    ...
    """System-level quote that allows only the live the internal simulation engine feed (plus Redis snapshots), never Yahoo/demo."""
    fmt = _format_symbol(symbol)
    market_frozen = _is_market_frozen()
    ...
    async def _fetch():
        ...
        normalized = await _align_quote_to_history(fmt, result)
        if normalized:
            normalized.setdefault("source", "tickalpha")
            return _adjust_for_market_state(normalized)
        return None
...
async def get_historical_data_live_only(
    ...
    """Historical candles from the live the internal simulation engine provider only (with Redis snapshot fallback)."""
    symbol = _format_symbol(symbol)
...
async def get_historical_data(
    ...
    """
    Get historical OHLCV data for charts via live the internal simulation engine data only.
    """
    symbol = _format_symbol(symbol)
    ...
    async def _fetch_history():
        ...
        try:
            ...
            if normalized and is_index and is_intraday and not _history_has_volume(normalized):
                try:
                    normalized = _convert_monotonic_volumes_to_deltas(normalized)
                except Exception:
                    pass
                if normalized:
                    logger.debug(
...
async def search_stocks(query: str) -> list:
    """Search for Indian stocks — local + the internal simulation engine symbol registry.

    Priority order:
    1. Local NSE list (~400 stocks, instant, prefix-ranked)
    2. the internal simulation engine symbol registry (real exchange data, covers ALL NSE stocks)
    ...
    # ── Step 2: Remote search (the internal simulation engine symbol registry) ─────────────────────
    internal_sim_results = await _search_internal_sim(query_upper)
    remote_results = internal_sim_results if isinstance(internal_sim_results, list) else []
    ...
    # Then remote results (the internal simulation engine symbol registry)
...
async def _search_internal_sim(query: str) -> list:
    """Search NSE equity symbols in the internal simulation engine symbol registry (populated
    at startup from GET /v1/symbols/NSE — see providers/contract_symbol_map.py)."""
    try:
        from providers.contract_symbol_map import search_registered_symbols

        matches = search_registered_symbols(query, segment="EQ", limit=15)
        return [
            {
                "symbol": m["canonical"],
                "name": m["symbol"],
        ...
        logger.debug(f"the internal simulation engine symbol registry search failed: {e}")
        return []
...
async def get_ticker_data(user_id: Optional[str] = None) -> list:
    ...
    All symbols fetched in parallel via live the internal simulation engine data only.
    """
    import asyncio as _asyncio
...
async def get_batch_quotes(symbols: list[str], user_id: Optional[str] = None) -> dict:
    """
    Get quotes for multiple symbols via live the internal simulation engine data only.
    ...
    async def _fetch_batch():
        # Prefer Redis live batch first (streamed from the internal simulation engine feed).
        ...
        # Register exact the internal simulation engine symbol mappings before live provider fetch.
        try:
...
async def get_public_ticker_data() -> list:
    """Get ticker data via the shared data feed provider session."""
...
async def get_commodity_quotes(user_id: Optional[str] = None) -> list:
    """Fetch commodity quotes from live the internal simulation engine discovery, with Redis snapshot fallback only."""
    ...
    async def _fetch_one(commodity: dict) -> Optional[dict]:
        ...
        # the internal simulation engine-only snapshot fallback (Redis/last frozen quote path), never demo/NSE.
        if not live_quote:
            try:
        ...
        if isinstance(live_quote, dict) and live_quote.get("price") is not None:
            ...
            live_quote["source"] = str(live_quote.get("source") or "live_tickalpha")
            if not market_open:
    ...
    if cached_by_symbol:
        logger.warning("Returning cached commodity data (the internal simulation engine live feed unavailable)")
        return list(cached_by_symbol.values())
