...
async def place_futures_order(
    ...
    async def _live_futures_ltp(symbol: str) -> Optional[Decimal]:
        """Live LTP from the futures data feed pipeline only (never equity quote cache)."""
    ...
    # LIMIT / STOP: always use a fresh live futures quote (ignore stale UI client_price).
    if order_type == "LIMIT" or order_type == "STOP_LOSS_LIMIT":
        current_price = await _live_futures_ltp(contract_symbol)
        if current_price is None:
            return {
                "success": False,
                "error": "Unable to fetch a live market price for this contract",
            }
        client_price = None
    else:
        current_price = await _live_futures_ltp(contract_symbol)
        if current_price is None:
            if client_price and client_price > 0:
                logger.info(
                    f"Using client-provided tick {client_price} for {contract_symbol}"
                )
