...
from data_layer.db.session import AsyncSessionLocal
from data_layer.db.models import PriceData, IngestionLog
from data_layer.ingestion.nse_bhavcopy import get_nse_data
from data_layer.ingestion.bse_bhavcopy import get_bse_data
from data_layer.ingestion.nse_fo_bhavcopy import get_nse_fo_data
from data_layer.ingestion.index_bhavcopy import get_index_data
    ...
    try:
        ...
        if use_mock:
            if exchange == "NSE":
                ...
                records = get_index_data(target_date, use_mock=True)
                mock_csv_content = "symbol,segment,date,open,high,low,close,volume\n" + \
                    "\n".join([f"{r['symbol']},{r['segment']},{target_date.isoformat()},{r['open']},{r['high']},{r['low']},{r['close']},{r['volume']}" for r in records])
                archive_raw_data("NSE_INDEX", target_date, "mock_index_bhavcopy.csv", mock_csv_content)
            else:
                records = []
        else:
            ...
            if exchange == "NSE":
                ...
                from data_layer.ingestion.nse_bhavcopy import download_nse_bhavcopy, parse_nse_bhavcopy_csv, zipfile, io
                try:
                ...
                from data_layer.ingestion.bse_bhavcopy import download_bse_bhavcopy, parse_bse_bhavcopy_csv
                try:
                ...
                from data_layer.ingestion.nse_fo_bhavcopy import download_nse_fo_bhavcopy, parse_nse_fo_bhavcopy_csv, zipfile, io
                try:
                ...
                # No public free bhavcopy exists for index-level EOD OHLCV;
                # index rows are always deterministically simulated.
                records = get_index_data(target_date, use_mock=True)
            else:
...
async def ingest_date(target_date: date, use_mock: bool, try_real_first: bool = False) -> Dict[str, int]:
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            return {"NSE": nse_rows, "BSE": bse_rows, "NSE_FO": nse_fo_rows, "NSE_INDEX": index_rows}
        except Exception as e:
...
async def run_backfill(days_to_backfill: int, use_mock: bool, try_real_first: bool = False):
    ...
    logger.info(
        ...
        f"Total NSE F&O records: {total_nse_fo}, Total Index records: {total_index}"
    )
