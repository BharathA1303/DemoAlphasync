...
function OptionsStrikeDetailsPanel({
  ...
  const isLive = isLiveChainSource(source);
  ...
  const waiting = isLive ? null : 'Waiting for live quote';
    ...
    <div className="flex flex-col h-full min-h-0 overflow-y-auto">
      ...
      <div className="px-3 py-2 flex-1">
        ...
        <DetailRow
          ...
          hint="From bid-ask vs mid (live feed)"
        />
        <DetailRow
          label="LTP"
          value={formatOptionPrice(sideData?.ltp, { source, bid: sideData?.bid, ask: sideData?.ask })}
          hint={waiting}
        />
        <DetailRow
          label="Change"
          value={formatOptionPct(sideData?.changePct, { source })}
          valueClassName={Number(sideData?.changePct) >= 0 ? 'text-bull' : 'text-bear'}
        />
        <DetailRow label="Bid" value={formatOptionPrice(sideData?.bid, { source })} />
        <DetailRow label="Ask" value={formatOptionPrice(sideData?.ask, { source })} />
        <DetailRow
          label="Spread"
          value={spread != null && isLive ? `₹${spread.toFixed(2)}` : '—'}
        />
        <DetailRow label="OI" value={formatOptionOi(sideData?.oi, { source })} />
        <DetailRow
          label="OI Change"
          value={oiPct != null ? formatOptionPct(oiPct, { source }) : '—'}
          valueClassName={oiPct != null && oiPct >= 0 ? 'text-bull' : 'text-bear'}
        />
        <DetailRow label="Volume" value={formatOptionOi(sideData?.volume, { source })} />
        ...
        {oppositeSideData && (
          <>
            ...
            <DetailRow label="LTP" value={formatOptionPrice(oppositeSideData?.ltp, { source })} />
            <DetailRow label="OI" value={formatOptionOi(oppositeSideData?.oi, { source })} />
            <DetailRow label="Volume" value={formatOptionOi(oppositeSideData?.volume, { source })} />
          </>
        )}
      </div>
    </div>
