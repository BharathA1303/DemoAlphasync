...
import { formatOptionPrice, formatOptionOi } from './formatOptionValue';
    ...
    <div className="flex-shrink-0 border-b border-edge/5 bg-surface-900/80 min-h-0">
      ...
      <div className="px-3 py-2 space-y-0.5 max-h-[280px] overflow-y-auto overscroll-contain">
        ...
        <OptionsAnalyticsCard
          ...
          value={
            analytics?.spotPrice != null
              ? `₹${formatOptionPrice(analytics.spotPrice, { source, allowZero: true })}`
              : '—'
          }
        />
        ...
        {showAtmIv && (
          <OptionsAnalyticsCard
            label="IV (ATM)"
            value={`${formatOptionPrice(analytics.atmIv, { source })}%`}
          />
        )}
        <OptionsAnalyticsCard label="Total CE OI" value={formatOptionOi(analytics?.totalCeOi, { source })} />
        <OptionsAnalyticsCard label="Total PE OI" value={formatOptionOi(analytics?.totalPeOi, { source })} />
        <OptionsAnalyticsCard
          label="Expiry"
