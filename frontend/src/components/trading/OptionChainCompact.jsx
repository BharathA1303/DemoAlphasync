...
import {
  formatOptionOi,
  formatOptionPctParen,
  formatOptionPrice,
  isLiveChainSource,
  oiChangePercent,
} from '../options/formatOptionValue';
...
function CallSide({ data, strike, selected, source, onSelect, spotPrice }) {
  ...
  return (
    <div className={cn('group relative grid grid-cols-2 h-full min-h-0', selected && 'ring-1 ring-inset ring-primary-500/25')}>
      <MetricCell
        value={formatOptionOi(data.oi, { source })}
        subValue={oiPct != null ? formatOptionPctParen(oiPct, { source }) : null}
        subUp={oiPct != null && oiPct >= 0}
        onClick={() => onSelect(strike, 'CE', { ...data, side: 'BUY' })}
        itmClass={itmClass}
      />
      <MetricCell
        value={formatOptionPrice(data.ltp, { source, noFallback: true })}
        subValue={Number.isFinite(ltpPct) ? formatOptionPctParen(ltpPct, { source }) : null}
...
function SpotDividerRow({ spotPrice, style }) {
  const label = formatOptionPrice(spotPrice, { source: 'tickalpha', allowZero: true });
  return (
    <div
...
function OptionChainCompact({
  ...
  return (
    <div className="option-chain-compact flex flex-col h-full min-h-0 bg-surface-900 dark:bg-surface-950">
