/**
 * LiveChart — TradingView Lightweight Charts candlestick chart.
 *
 * Uses lightweight-charts v4 for professional charting.
 * All OHLCV data comes from the backend market data REST API.
 * Live candle updates come from WebSocket ticks via the market store.
 *
 * Features:
...
const LiveChart = memo(function LiveChart({
    ...
    const [activeIndicators, setActiveIndicators] = useState(() => {
        try {
            const saved = localStorage.getItem('alphasync_chart_active_indicators');
    ...
    const [hiddenIndicators, setHiddenIndicators] = useState(() => {
        try {
            const saved = localStorage.getItem('alphasync_chart_hidden_indicators');
            return saved ? new Set(JSON.parse(saved)) : new Set();
        } catch {
            return new Set();
        }
    ...
    const [legendCollapsed, setLegendCollapsed] = useState(() => {
        try {
            const saved = localStorage.getItem('alphasync_chart_legend_collapsed');
            return saved === 'true';
        } catch {
            return false;
        }
    ...
    useEffect(() => {
        localStorage.setItem('alphasync_chart_active_indicators', JSON.stringify([...activeIndicators]));
    }, [activeIndicators]);

    useEffect(() => {
        localStorage.setItem('alphasync_chart_hidden_indicators', JSON.stringify([...hiddenIndicators]));
    }, [hiddenIndicators]);

    useEffect(() => {
        localStorage.setItem('alphasync_chart_legend_collapsed', String(legendCollapsed));
    }, [legendCollapsed]);
