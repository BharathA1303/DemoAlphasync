...
import DataFeedPanel from './DataFeedPanel';
...
export default function AdminPanelPage() {
    ...
    const [authStage, setAuthStage] = useState('dashboard');
    ...
    const handleSaveZebuCredentials = useCallback(async () => {
        setZebuSaving(true);
        try {
            await adminApi.saveZebuCredentials(zebuDraft);
            toast.success('Zebu credentials saved');
            await loadZebuStatus();
            setZebuDraft(prev => ({ ...prev, password: '', factor2: '', api_secret: '' }));
        } catch (err) {
            toast.error(parseApiError(err, 'Failed to save Zebu credentials'));
        } finally {
            setZebuSaving(false);
        }
    }, [zebuDraft, loadZebuStatus]);

    const handleImportZebuHistory = useCallback(async () => {
        setZebuImporting(true);
        try {
    ...
    const resetToVerifyStage = useCallback((message = 'Your session has expired. Please sign in again.') => {
        setModalUserId(null);
    ...
    async function runUserAction(actionName, payload) {
        ...
        try {
            if (actionName === 'approve') { await adminApi.approveUser(modalUser.id, durationDays); toast.success('User approved'); }
                ...
                await adminApi.deleteUserAccount(modalUser.id);
            ...
            if ([401, 403].includes(err?.response?.status)) resetToVerifyStage();
    ...
    return (
        <div className="page-container">
                                                        ...
                                                        <div className="flex flex-col gap-2">
                                                            ...
                                                            <button
                                                                ...
                                                                onClick={async () => {
                                                                    ...
                                                                    try {
                                                                        await adminApi.deleteUserAccount(u.id);
                                                                        toast.success('User account permanently deleted');
                                                                        await refreshDashboard();
                                                                    } catch (err) {
                                                                        if ([401, 403].includes(err?.response?.status)) {
            ...
            {showDataFeedModal && canManage && (
                <DataFeedModal onClose={() => setShowDataFeedModal(false)} />
            )}
        </div>
    );
}
