/**
 * Options workstation — compact option chain (left), chart (center), details (right),
...
import { isLiveChainSource } from '../components/options/formatOptionValue';
...
export default function OptionsPage() {
  ...
  /** Selected option contract OHLC (contract tsym) — not underlying index. */
  ...
  const applyChainPayload = useCallback((payload, { silent = false } = {}) => {
    ...
    setChainSource(src || 'tickalpha_cache');
  ...
  const fetchChain = useCallback(async (symbol, expiry, { silent = false } = {}) => {
    ...
    try {
      ...
      if (!isLiveChainSource(src)) {
        if (!silent) {
          setOptionChain([]);
          setUnderlyingPrice(0);
          setError('Option chain requires the TickAlpha live data feed. No fallback quotes shown — check with your admin that the data feed is connected.');
        }
        return;
      }
      ...
      if (!silent) {
        ...
        setError(detail ?? 'Failed to fetch option chain.');
        setOptionChain([]);
      }
    } finally {
  ...
  useEffect(() => {
    if (!nearestRow || !isLiveChainSource(chainSource)) return;
  ...
  useEffect(() => {
    if (!sideData || !isLiveChainSource(chainSource)) return;
  ...
  const handlePlaceOrder = useCallback(() => {
    ...
    if (!isLiveChainSource(chainSource) || premiumValue <= 0) {
      toast.error('Valid live premium required before placing order');
      return;
    }
  ...
  const orderPanelEl = (
    <OptionsOrderPanel
      ...
      disabled={!selectedStrike || !isLiveChainSource(chainSource) || premiumValue <= 0}
    />
  );
