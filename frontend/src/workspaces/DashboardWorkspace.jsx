...
export default function DashboardWorkspace() {
    ...
    const selectedIndexQuote = useMemo(() => {
        return indices.find(idx => idx.symbol === activeTab) || null;
    }, [indices, activeTab]);

    const chartCandles = useMemo(() => {
        if (!(history && history.length > 1)) return [];

        const baseCandles = history.map(c => ({ ...c }));
        // Dynamically sync the last candle with the real-time quote price to make it animate
        if (selectedIndexQuote?.price) {
            baseCandles[baseCandles.length - 1].close = selectedIndexQuote.price;
        }
        return baseCandles;
    }, [history, selectedIndexQuote]);
